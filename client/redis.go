...
func DialWithConfig(cfg *DialConfig) (*Redis, error) {
	...
	r := &Redis{
		...
		timeout:       cfg.Timeout,
		tcp_keepalive: cfg.TCPKeepAlive,
	}
	r.pool = &connPool{
		MaxIdle: cfg.MaxIdle,
		Dial:    r.dialConnection,
...
func DialURL(rawurl string) (*Redis, error) {
	...
	tcp_keepalive, err := strconv.Atoi(ul.Query().Get("tcp_keepalive"))
	if err != nil {
		return nil, err
	}
	return DialWithConfig(&DialConfig{ul.Scheme, ul.Host, db, password, timeout, maxidle, tcp_keepalive})
}
