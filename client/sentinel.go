...
// SentinelReset executes the SENTINEL RESET command on the server
// This is used to reset the sentinel's knowledge of the pod
func (r *Redis) SentinelReset(podname string) error {
	_, err := r.ExecuteCommand("SENTINEL", "RESET", podname)
	return err
}
